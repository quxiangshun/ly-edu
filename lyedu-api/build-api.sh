#!/bin/bash
# LyEdu API 构建脚本（Linux/Mac）
# 在 lyedu-api 目录下执行: ./build-api.sh
# 使用 Gradle 构建并复制 jar 到仓库根目录 pkg/

echo "========================================"
echo "LyEdu API - Gradle 构建"
echo "========================================"
echo ""

API_DIR="$(cd "$(dirname "$0")" && pwd)"
PKG_DIR="$(cd "$API_DIR/.." && pwd)/pkg"

if [ ! -d "$API_DIR" ]; then
    echo "错误: 脚本所在目录无效"
    exit 1
fi

mkdir -p "$PKG_DIR"
cd "$API_DIR" || exit 1

# 检查 Gradle Wrapper
if [ ! -f "./gradlew" ]; then
    echo "错误: 找不到 Gradle Wrapper"
    echo "请先运行: .\init-gradle.ps1 (Windows) 或 gradle wrapper (Linux/Mac)"
    exit 1
fi

echo "开始构建..."
echo ""

./gradlew bootJar --rerun-tasks --no-build-cache

if [ $? -ne 0 ]; then
    echo "构建失败！"
    exit 1
fi

echo ""
echo "构建成功！"

JAR_PATH="$PKG_DIR/lyedu-api.jar"
if [ -f "$JAR_PATH" ]; then
    echo "JAR 文件位置: $JAR_PATH"
    echo "文件大小: $(du -h "$JAR_PATH" | cut -f1)"
else
    echo "警告: 未在 pkg 目录找到 JAR，请检查构建输出"
fi

echo ""
echo "下一步："
echo "  cd scripts/docker"
echo "  docker compose build api"
echo "  docker compose up -d"
echo ""

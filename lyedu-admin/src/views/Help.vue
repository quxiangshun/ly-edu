<template>
  <div class="help-page">
    ...
    <div class="help-content">
      ...
      <p>用于维护公司的组织架构，支持多级部门。</p>
      <p>支持为部门打标签：在新增/编辑部门时可多选标签，或点击「关联标签」为部门关联标签，列表中会展示该部门的标签名称。</p>
      ...
      <p>支持为课程打标签：在新增/编辑课程时可多选标签，<strong>标签为必选项</strong>，列表中会展示该课程的标签名称。<strong>关联部门为选填</strong>，私有课程可不关联部门，仅通过标签控制可见范围。<strong>学员端可见性</strong>：用户能看到某课程的条件为（满足其一即可）——该课程为公开；或该课程为私有且关联了用户所属部门/子部门；或该课程带有「用户有效标签」中的任意一个（用户有效标签 = 用户自身标签 ∪ 用户所属部门标签 ∪ 用户所属部门及其子部门标签）。详见上方「标签管理」。</p>
      <ul>
        <li>课程下可维护章节与视频：章节用于组织课程结构，视频管理中上传的视频可挂载到课程章节或“未分类”。</li>
        <li>可见性支持公开/私有：私有课程可关联部门（选填），也可通过标签控制可见范围（见标签管理）；标签必选。</li>
      ...
      <p>维护题库（单选、多选、判断、填空、简答），供试卷和考试引用。</p>
      <h4>新增试题</h4>
      <p>点击「新增试题」在弹窗中单条录入：选择题型，填写题干、选项（单选/多选/判断需填 JSON 数组）、参考答案、分值、解析、排序。</p>
      <h4>上传试题</h4>
      <p>支持三种方式批量导入，表头/字段一致：<strong>题型、题干、选项(JSON)、参考答案、分值、解析、排序</strong>。题型取值为：单选、多选、判断、填空、简答。</p>
      <ul>
        <li><strong>下载模板</strong>：在「下载上传模板」下拉中可选 Excel(.xlsx)、CSV(.csv)、JSON(.json) 模板，内含五种题型各一条示例，按模板格式填写后再导入。</li>
        <li><strong>上传文件</strong>：在「上传试题」弹窗的「上传文件」标签下选择 .xlsx / .csv / .json 文件后，系统会先<strong>预览</strong>解析出的试题列表（题型、题干、参考答案、分值、排序）。确认无误后点击「提交」，在二次确认对话框中确认后才会真正执行导入。</li>

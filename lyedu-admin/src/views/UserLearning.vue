<template>
  <div class="user-learning-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <div class="card-header-left">
            <span>用户学习记录</span>
            <el-tooltip content="查看所有用户的学习记录和进度" placement="right">
              <el-icon class="card-help-icon" @click="openPageHelp('user-learning')">
                <QuestionFilled />
              </el-icon>
            </el-tooltip>
          </div>
      ...
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="用户名/姓名/课程名称" clearable />
        </el-form-item>
      ...
      <el-table :data="learningList" v-loading="loading" border :max-height="tableMaxHeight">
        ...
        <el-table-column prop="completeStatus" label="完成状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="row.completeStatus === 1 ? 'success' : 'info'">
              {{ row.completeStatus === 1 ? '已完成' : '学习中' }}
            </el-tag>
          </template>
        </el-table-column>
...
import { getUserLearningPage, type UserLearningRecord } from '@/api/userLearning'
...
const learningList = ref<UserLearningRecord[]>([])
...
async function loadLearningRecords() {
  loading.value = true
  try {
    const res = await getUserLearningPage({
